...
1. Point the production domain's `A` record at the OCI public IP. Add an `AAAA` record only when IPv6 is configured. Allow inbound TCP 22, 80, and 443 in both the OCI security list/NSG and host firewall; do not expose the API's port 5001.
...
3. Create `/var/www/naughty-jars`, owned by the application user. Store the deployment checkout in `/var/www/naughty-jars/deployer`; the scripts manage `releases`, `current`, and `previous` alongside it. Create `/var/www/naughty-jars/backend.env`, owned by the application user with mode `0600`. Populate it from `.env.example` using production values, `PORT=5001`, and a newly generated JWT secret. Caddy serves only `current/frontend/dist`, never the directory containing this environment file.
...
5. Preserve any existing Caddy sites. Copy `deploy/Caddyfile` to `/etc/caddy/sites/naughty-jars.caddy`, and add `import /etc/caddy/sites/*.caddy` to the existing `/etc/caddy/Caddyfile` if it does not already import that directory. Create the log directory with `sudo install -d -o caddy -g caddy /var/log/caddy`, then add a systemd override with `sudo systemctl edit caddy`:
   ...
   Replace the hostname, run `sudo systemctl daemon-reload`, validate with `sudo -u caddy env NAUGHTY_JARS_SITE_ADDRESS=example.com caddy validate --config /etc/caddy/Caddyfile`, and reload Caddy. Caddy must be able to traverse `/var/www/naughty-jars/current/frontend/dist`.
...
```bash
ENV_FILE=/var/www/naughty-jars/backend.env npm run audit:media
```
...
```bash
ENV_FILE=/var/www/naughty-jars/backend.env npm run migrate:media
ENV_FILE=/var/www/naughty-jars/backend.env npm run audit:media -- --require-clean
```
